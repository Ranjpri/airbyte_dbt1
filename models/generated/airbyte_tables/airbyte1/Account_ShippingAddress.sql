  ...
  create or replace table `technology-dev-tab`.airbyte1.`Account_ShippingAddress`
  partition by timestamp_trunc(_airbyte_emitted_at, day)
  cluster by _airbyte_emitted_at
  OPTIONS()
  as (
    
with __dbt__cte__Account_ShippingAddress_ab1 as (
...
-- ShippingAddress at Account/ShippingAddress
where 1 = 1
and ShippingAddress is not null
),  __dbt__cte__Account_ShippingAddress_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__Account_ShippingAddress_ab1
select
    _airbyte_Account_hashid,
    cast(city as 
    string
    ...
    _airbyte_emitted_at,
    CURRENT_TIMESTAMP() as _airbyte_normalized_at
from __dbt__cte__Account_ShippingAddress_ab1
-- ShippingAddress at Account/ShippingAddress
where 1 = 1
),  __dbt__cte__Account_ShippingAddress_ab3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__Account_ShippingAddress_ab2
select
    ...
    tmp.*
from __dbt__cte__Account_ShippingAddress_ab2 tmp
-- ShippingAddress at Account/ShippingAddress
where 1 = 1
)-- Final base SQL model
-- depends_on: __dbt__cte__Account_ShippingAddress_ab3
select
    _airbyte_Account_hashid,
    city,
    state,
...
from __dbt__cte__Account_ShippingAddress_ab3
-- ShippingAddress at Account/ShippingAddress from `technology-dev-tab`.airbyte1.`Account`
where 1 = 1
  );
