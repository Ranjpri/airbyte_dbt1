  ...
  create or replace table `technology-dev-tab`.airbyte1.`Contract_BillingAddress`
  partition by timestamp_trunc(_airbyte_emitted_at, day)
  cluster by _airbyte_emitted_at
  OPTIONS()
  as (
    
with __dbt__cte__Contract_BillingAddress_ab1 as (
...
-- BillingAddress at Contract/BillingAddress
where 1 = 1
and BillingAddress is not null
),  __dbt__cte__Contract_BillingAddress_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__Contract_BillingAddress_ab1
select
    _airbyte_Contract_hashid,
    cast(city as 
    string
    ...
    _airbyte_emitted_at,
    CURRENT_TIMESTAMP() as _airbyte_normalized_at
from __dbt__cte__Contract_BillingAddress_ab1
-- BillingAddress at Contract/BillingAddress
where 1 = 1
),  __dbt__cte__Contract_BillingAddress_ab3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__Contract_BillingAddress_ab2
select
    ...
    tmp.*
from __dbt__cte__Contract_BillingAddress_ab2 tmp
-- BillingAddress at Contract/BillingAddress
where 1 = 1
)-- Final base SQL model
-- depends_on: __dbt__cte__Contract_BillingAddress_ab3
select
    _airbyte_Contract_hashid,
    city,
    state,
...
from __dbt__cte__Contract_BillingAddress_ab3
-- BillingAddress at Contract/BillingAddress from `technology-dev-tab`.airbyte1.`Contract`
where 1 = 1
  );
